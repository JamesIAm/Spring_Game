...
class Player(GomokuAgent):
    def __init__(self, ID, BOARD_SIZE, X_IN_A_LINE):
        ...
        self.defineOrders()
        self.MAX_DEPTH = 10 #Maximum depth the program will search to. Must be greater than 1
        self.NEXT_PLAYER_WEIGHTS =[0,0,0,0,0,1000,0,0,2,10,500,1000,0,0,10,100,500,1000]#Indicates the weights that scores are multiplied by.
        ...
        self.CURRENT_PLAYER_WEIGHTS =[0,0,0,0,0,1000,0,0,2,10,50,1000,0,0,10,50,200,1000]
        self.SEARCH_RADIUS = 1 # How far around the current min and max co-ords the system looks for new moves. At 1, a tile on 3,3 will result in a search of 2,2 to 4,4
        self.INDEXES = [(X_IN_A_LINE+1)-1, ((X_IN_A_LINE+1)*2)-1, ((X_IN_A_LINE+1)*3)-1] # These indicate, of all the scores, 
        ...
        self.SEARCH_TIME = 4.7 #How many seconds the system searches before returning it's best solution
        # self.counter = 0
        self.priorityMoves = []#Contains a list of moves that should be searched first (continuations of the line of the last move played)
    ...
    def findMyMove(self, board, depth, maxDepth, alpha, beta, prevThisPlayerScore, prevOtherPlayerScore, priorityMoves):
    ...
    def findTheirMove(self, board, depth, maxDepth, alpha, beta, prevThisPlayerScore, prevOtherPlayerScore, priorityMoves):
        if (time() > (self.startTime + self.SEARCH_TIME)):
                return 0, None
        if (depth < maxDepth):
            validMoves = self.findMoves(board, priorityMoves.copy())
    ...
    def simulateMove(self, board, x, y, id):
    ...
    def countLinesInDirection(self, board, orderedItems, id):
    ...
    def countChangeAdd(self, id, prevBoard, prevScore, move):
        self.priorityMoves = []
        score = prevScore
        y = move[0]
        x = move[1]
    ...
    def calculateNewLines(self, board, id, x, y, prevScore, length, openess, xChange, yChange): 
    ...
    def countChangeMinus(self, id, prevBoard, prevScore, move):
        self.priorityMoves = []
        score = prevScore
        y = move[0]
        x = move[1]
    ...
    def calculateOldLines(self, board, id, x, y, prevScore, xChange, yChange): 
    ...
    def findMoves(self, board, priorityMoves):
